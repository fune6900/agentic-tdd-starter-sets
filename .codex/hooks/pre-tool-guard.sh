...
#
#   [shell] シェルが実行して初めて危険なもの（rm / git / chmod / mkfs / dd / sudo …）
...
#           → クォートは取り除かない。そのプログラムが登場するときだけ検査する
#
# **検知パターンは1つも減らしていない。**
# 詳細と既知の限界は .claude/hooks/pre-tool-guard.sh のコメントを参照。
...
#
#   inert  … cat / tee。本文は実行されない → 常に落とす
#   shell  … sh / bash / zsh / dash / ssh。本文はシェルとして実行される → 常に残す
#   other  … python / node / psql など。本文は別言語として実行される
#            → シェル系パターンの走査からは外し、引数系（SQL）には残す
#
# 第1引数 shell: inert と other の本文を落とす / args: inert の本文だけを落とす
strip_heredocs() {
  local mode="${1:-shell}"
    ...
    END {
      ...
      while (i <= n) {
        ...
        if (c == "\"") {
          ...
          if (closed == 0) { out = out substr(s, i); break }
