...
> Claude Code 向け TDD駆動・サブエージェントオーケストレーション・スターターテンプレート

「テストのないコードは存在しない」を強制する、Claude Code 用のルールセット・サブエージェント定義・スラッシュコマンドの再利用可能なテンプレート。
...
```bash
# プロジェクトのルートで
git clone https://github.com/<your-account>/claude-tdd-starter.git /tmp/claude-tdd-starter
cp -r /tmp/claude-tdd-starter/.claude .
cp /tmp/claude-tdd-starter/CLAUDE.md .
rm -rf /tmp/claude-tdd-starter
```

または degit:

```bash
npx degit <your-account>/claude-tdd-starter#main .claude-tdd-starter-temp
cp -r .claude-tdd-starter-temp/.claude .
cp .claude-tdd-starter-temp/CLAUDE.md .
rm -rf .claude-tdd-starter-temp
```

### 2. プロジェクトに合わせて編集する

- `CLAUDE.md` 内の `{{PROJECT_NAME}}` / `{{PROJECT_DESCRIPTION}}` を書き換える
...
| `/merge-and-sync`    | PRをmainにマージしてローカルを同期              |
| `/coderabbit-fix`    | CodeRabbitの指摘を取得・自動修正                |
| `/e2e-test`          | E2Eテスト実行（Playwright MCP）                 |
| `/visual-regression` | 視覚的整合性検証                                |
